...
#         temp_typ = typ
#         print(typ)
#         if "RVec<float>" in str(typ):temp_typ = 'float'
#         elif "RVec<double>" in str(typ):temp_typ = 'double'
#         elif "RVec<int>" in str(typ): temp_typ = 'int'
#         tree._ttras[branchName] = _makeArrayReader(tree, temp_typ, branchName)
#     return tree._ttras[branchName]
...
def readBranch(tree, branchName):
    ...
    if branchName in tree._extrabranches:
        ...
        typ = leaf.GetTypeName()
        if "RVec" in str(typ):
            if "RVec<float>" in str(typ):typRVec = 'float'
            elif "RVec<int>" in str(typ): typRVec = 'int'
            elif "RVec<double>" in str(typ):typRVec = 'double'
            # print(f"{typRVec=}")
            _ar = _makeArrayReader(tree, typRVec, branchName)
            # force calling SetEntry as a new ArrayReader was created
            tree.gotoEntry(tree.entry, forceCall=True)
            return _ar
        elif leaf.GetLen() == 1 and not bool(leaf.GetLeafCount()):
